...
async function runMatrixEntry(
  ...
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'metaclaw-ci-'));
  ...
  try {
    ...
    // Initialize metaclaw dir
    execSync('npx tsx -e "import { initMetaclawDir } from \'./skills-engine/index.js\'; initMetaclawDir();"', {
      cwd: tmpDir,
      stdio: 'pipe',
      timeout: 30_000,
    });
