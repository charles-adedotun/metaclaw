...
function setupLaunchd(projectRoot: string, nodePath: string, homeDir: string): void {
  const plistPath = path.join(homeDir, 'Library', 'LaunchAgents', 'com.metaclaw.plist');
...
<dict>
    ...
    <string>${projectRoot}/logs/metaclaw.log</string>
    <key>StandardErrorPath</key>
    <string>${projectRoot}/logs/metaclaw.error.log</string>
</dict>
</plist>`;
  ...
  try {
    ...
    serviceLoaded = output.includes('com.metaclaw');
  } catch {
...
/**
 * Kill any orphaned metaclaw node processes left from previous runs or debugging.
...
function killOrphanedProcesses(projectRoot: string): void {
  try {
    ...
    logger.info('Stopped any orphaned metaclaw processes');
  } catch {
...
function setupSystemd(projectRoot: string, nodePath: string, homeDir: string): void {
  ...
  if (runningAsRoot) {
    unitPath = '/etc/systemd/system/metaclaw.service';
    ...
    unitPath = path.join(unitDir, 'metaclaw.service');
    systemctlPrefix = 'systemctl --user';
  }

  const unit = `[Unit]
Description=MetaClaw Personal Assistant
...
StandardOutput=append:${projectRoot}/logs/metaclaw.log
StandardError=append:${projectRoot}/logs/metaclaw.error.log
...
WantedBy=${runningAsRoot ? 'multi-user.target' : 'default.target'}`;
  ...
  // Kill orphaned metaclaw processes to avoid WhatsApp conflict errors
  ...
  try {
    execSync(`${systemctlPrefix} enable metaclaw`, { stdio: 'ignore' });
  } catch (err) {
    logger.error({ err }, 'systemctl enable failed');
  }

  try {
    execSync(`${systemctlPrefix} start metaclaw`, { stdio: 'ignore' });
  } catch (err) {
  ...
  let serviceLoaded = false;
  try {
    execSync(`${systemctlPrefix} is-active metaclaw`, { stdio: 'ignore' });
    serviceLoaded = true;
  } catch {
    // Not active
  }
...
function setupNohupFallback(projectRoot: string, nodePath: string, homeDir: string): void {
  ...
  const wrapperPath = path.join(projectRoot, 'start-metaclaw.sh');
  const pidFile = path.join(projectRoot, 'metaclaw.pid');

  const lines = [
    '#!/bin/bash',
    '# start-metaclaw.sh — Start MetaClaw without systemd',
    ...
    '    echo "Stopping existing MetaClaw (PID $OLD_PID)..."',
    ...
    `  >> ${JSON.stringify(projectRoot + '/logs/metaclaw.log')} \\`,
    `  2>> ${JSON.stringify(projectRoot + '/logs/metaclaw.error.log')} &`,
    '',
    `echo $! > ${JSON.stringify(pidFile)}`,
    'echo "MetaClaw started (PID $!)"',
    `echo "Logs: tail -f ${projectRoot}/logs/metaclaw.log"`,
  ];
