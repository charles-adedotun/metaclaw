...
function parseArgs(args: string[]): RegisterArgs {
  ...
  for (let i = 0; i < args.length; i++) {
    switch (args[i]) {
      ...
      case '--assistant-name': result.assistantName = args[++i] || 'Assistant'; break;
    }
  }
...
export async function run(args: string[]): Promise<void> {
  ...
  if (parsed.assistantName !== 'Assistant') {
    logger.info({ from: 'Assistant', to: parsed.assistantName }, 'Updating assistant name');
    ...
    for (const mdFile of mdFiles) {
      if (fs.existsSync(mdFile)) {
        ...
        content = content.replace(/^# Assistant$/m, `# ${parsed.assistantName}`);
        content = content.replace(/You are Assistant/g, `You are ${parsed.assistantName}`);
