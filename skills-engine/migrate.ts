...
import { BASE_DIR, CUSTOM_DIR, METACLAW_DIR } from './constants.js';
import { initMetaclawDir } from './init.js';
import { recordCustomModification } from './state.js';

export function initSkillsSystem(): void {
  initMetaclawDir();
  console.log('Skills system initialized. .metaclaw/ directory created.');
}
  ...
  try {
    ...
    if (diff.trim()) {
      ...
      const filesModified = [...diff.matchAll(/^diff -ruN .+ (.+)$/gm)]
        ...
        .filter((f) => !f.startsWith('.metaclaw'));
      ...
      console.log(
        'Custom modifications captured in .metaclaw/custom/migration.patch',
      );
    } else {
