...
export function createTempDir(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'metaclaw-test-'));
}

export function setupMetaclawDir(tmpDir: string): void {
  fs.mkdirSync(path.join(tmpDir, '.metaclaw', 'base', 'src'), { recursive: true });
  fs.mkdirSync(path.join(tmpDir, '.metaclaw', 'backup'), { recursive: true });
}
