...
import { createTempDir, setupMetaclawDir, cleanup } from './test-helpers.js';
...
describe('backup', () => {
  ...
  it('clearBackup removes backup directory', () => {
    ...
    const backupDir = path.join(tmpDir, '.metaclaw', 'backup');
  ...
  it('createBackup writes tombstone for non-existent files', () => {
    ...
    const tombstone = path.join(tmpDir, '.metaclaw', 'backup', 'src', 'newfile.ts.tombstone');
