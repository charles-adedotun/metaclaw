...
import { cleanup, createTempDir, initGitRepo, setupMetaclawDir } from './test-helpers.js';
...
describe('update', () => {
  ...
  function writeStateFile(state: Record<string, unknown>): void {
    const statePath = path.join(tmpDir, '.metaclaw', 'state.yaml');
        ...
        custom_modifications: [
          {
            ...
            patch_file: '.metaclaw/custom/001-tweak.patch',
          },
        ],
      });
  ...
  describe('applyUpdate', () => {
    it('rejects when customize session is active', async () => {
      ...
      const customDir = path.join(tmpDir, '.metaclaw', 'custom');
