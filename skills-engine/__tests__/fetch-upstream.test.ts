...
describe('fetch-upstream.sh', () => {
  ...
  beforeEach(() => {
    ...
    upstreamBareDir = fs.mkdtempSync(
      path.join(os.tmpdir(), 'metaclaw-upstream-'),
    );
    ...
    const seedDir = fs.mkdtempSync(
      path.join(os.tmpdir(), 'metaclaw-seed-'),
    );
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({ name: 'metaclaw', version: '2.0.0' }),
    );
    ...
    // Push as 'main' regardless of the local branch name so the bare repo
    // always has a 'main' branch. Without HEAD:main the push falls back to
    // 'master' on runners where init.defaultBranch is unset, causing
    // `git fetch upstream main` in the script to fail.
    ...
    projectDir = fs.mkdtempSync(
      path.join(os.tmpdir(), 'metaclaw-project-'),
    );
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({ name: 'metaclaw', version: '1.0.0' }),
    );
  ...
  it('uses existing upstream remote', () => {
    ...
    expect(status.TEMP_DIR).toMatch(/^\/tmp\/metaclaw-update-/);
