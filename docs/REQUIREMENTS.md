...
MetaClaw gives you the core functionality without that mess.
...
- Trigger: `@Assistant` prefix (case insensitive), configurable via `ASSISTANT_NAME` env var
...
- Custom `metaclaw` MCP server (inside container) provides scheduling tools
...
- **Trigger**: `@Assistant` (case insensitive)
- **Response prefix**: `Assistant:`
...
**MetaClaw** - A reference to Clawdbot (now OpenClaw).
