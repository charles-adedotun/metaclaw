---
name: x-integration
description: X (Twitter) integration for MetaClaw. Post tweets, like, reply, retweet, and quote. Use for setup, testing, or troubleshooting X functionality. Triggers on "setup x", "x integration", "twitter", "post tweet", "tweet".
---
...
> **Compatibility:** MetaClaw v1.0.0. Directory structure may change in future versions.
...
1. **MetaClaw is installed and running** - WhatsApp connected, service active
...
# Verify: launchctl list | grep metaclaw (macOS) or systemctl --user status metaclaw (Linux)
```
...
| `METACLAW_ROOT` | `process.cwd()` | Project root directory |
...
| `logs/metaclaw.log` | Service logs (contains X operation logs) | Ignored |
...
To integrate this skill into MetaClaw, make the following modifications:
...
All paths below are relative to project root (`METACLAW_ROOT`).
...
**Verify success:**
```bash
launchctl list | grep metaclaw  # macOS — should show PID and exit code 0 or -
# Linux: systemctl --user status metaclaw
```
...
grep -i "x_post\|x_like\|x_reply\|handleXIpc" logs/metaclaw.log | tail -20

# Script errors
grep -i "error\|failed" logs/metaclaw.log | tail -20
```
...
docker run metaclaw-agent ls -la /app/src/skills/
```
