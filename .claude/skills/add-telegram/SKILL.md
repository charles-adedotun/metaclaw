...
This skill adds Telegram support to MetaClaw using the skills engine for deterministic code changes, then walks through interactive setup.

## Phase 1: Pre-flight

### Check if already applied

Read `.metaclaw/state.yaml`. If `telegram` is in `applied_skills`, skip to Phase 3 (Setup). The code changes are already in place.
...
>    - Bot name: Something friendly (e.g., "Your Assistant")
...
> - For non-main: `@Assistant hello` or @mention the bot
>
...
4. Service is running: `launchctl list | grep metaclaw` (macOS) or `systemctl --user status metaclaw` (Linux)
...
- Check bot is started: `tail -f logs/metaclaw.log`
...
# Linux:
# systemctl --user stop metaclaw
# npm run dev
# systemctl --user start metaclaw
```
...
8. Rebuild: `npm run build && launchctl kickstart -k gui/$(id -u)/com.metaclaw` (macOS) or `npm run build && systemctl --user restart metaclaw` (Linux)
