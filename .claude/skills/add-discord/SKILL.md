...
This skill adds Discord support to MetaClaw using the skills engine for deterministic code changes, then walks through interactive setup.

## Phase 1: Pre-flight

### Check if already applied

Read `.metaclaw/state.yaml`. If `discord` is in `applied_skills`, skip to Phase 3 (Setup). The code changes are already in place.
...
> 2. Click **New Application** and give it a name (e.g., "Your Assistant")
...
4. Service is running: `launchctl list | grep metaclaw`
...
- @mention translation (Discord `<@botId>` → MetaClaw trigger format)
