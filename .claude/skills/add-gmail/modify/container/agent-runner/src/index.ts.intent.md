...
- Added: `gmail` MCP server alongside the existing `metaclaw` server:
  ```
  gmail: {
    command: 'npx',
...
## Invariants
- The `metaclaw` MCP server configuration is unchanged
...
## Must-keep
- The `metaclaw` MCP server with its environment variables
