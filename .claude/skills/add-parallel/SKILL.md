...
Adds Parallel AI MCP integration to MetaClaw for advanced web research capabilities.
...
- **Non-blocking Design** - Uses MetaClaw scheduler for result polling (no container blocking)

## Prerequisites

User must have:
1. Parallel AI API key from https://platform.parallel.ai
2. MetaClaw already set up and running
...
Add Parallel AI MCP servers after the metaclaw server:
```typescript
const mcpServers: Record<string, any> = {
  metaclaw: ipcMcp
};
...
3. Create a polling scheduled task using `mcp__metaclaw__schedule_task`:
   ```
   ...
   3. Send results to user with mcp__metaclaw__send_message
   4. Use mcp__metaclaw__complete_scheduled_task to mark this task as done
...
Verify the build:
```bash
echo '{}' | docker run -i --entrypoint /bin/echo metaclaw-agent:latest "Container OK"
```
...
```bash
sleep 3
launchctl list | grep metaclaw  # macOS
...
- Check task runs: `tail -f logs/metaclaw.log | grep "scheduled task"`
...
5. Restart: `launchctl kickstart -k gui/$(id -u)/com.metaclaw` (macOS) or `systemctl --user restart metaclaw` (Linux)
