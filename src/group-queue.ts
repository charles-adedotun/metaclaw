...
export class GroupQueue {
  ...
  /**
   * Check if any container is currently active for the given group folder.
   * Used by IPC watcher to suppress duplicate sends when streaming is active.
   */
  isActiveForFolder(groupFolder: string): boolean {
  ...
  sendMessage(groupJid: string, text: string): boolean {
    ...
    try {
      ...
      logger.error({ groupJid, err }, 'Failed to write IPC message to container');
      return false;
    }
  }
  ...
  closeStdin(groupJid: string): void {
    ...
    try {
      ...
      logger.warn({ groupJid, err }, 'Failed to write close sentinel to container');
    }
  }
