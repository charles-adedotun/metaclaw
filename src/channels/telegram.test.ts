...
vi.mock('../config.js', () => ({
  ...
  DATA_DIR: '/tmp/metaclaw-test-data',
  TIMEZONE: 'UTC',
}));
...
describe('TelegramChannel', () => {
  ...
  describe('non-text messages', () => {
    it('stores photo with placeholder', async () => {
      ...
      const ctx = createMediaCtx({
        extra: { photo: [{ file_id: 'photo123', width: 800, height: 600 }] },
      });
      await triggerMediaMessage('message:photo', ctx);

      expect(opts.onMessage).toHaveBeenCalledWith(
        'tg:100200300',
        expect.objectContaining({ content: expect.stringContaining('[Photo]') }),
      );
    });
    ...
    it('stores photo with caption', async () => {
      ...
      const ctx = createMediaCtx({
        ...
        extra: { photo: [{ file_id: 'photo456', width: 800, height: 600 }] },
      });
      await triggerMediaMessage('message:photo', ctx);

      // With download failure (mock), content is just the caption text
      expect(opts.onMessage).toHaveBeenCalledWith(
        'tg:100200300',
        expect.objectContaining({ content: expect.stringContaining('Look at this') }),
      );
    });
    ...
    it('stores video with placeholder', async () => {
      ...
      const ctx = createMediaCtx({
        extra: { video: { file_id: 'video123' } },
      });
      await triggerMediaMessage('message:video', ctx);

      expect(opts.onMessage).toHaveBeenCalledWith(
        'tg:100200300',
        expect.objectContaining({ content: expect.stringContaining('[Video]') }),
      );
    });
    ...
    it('stores voice message with placeholder', async () => {
      ...
      expect(opts.onMessage).toHaveBeenCalledWith(
        'tg:100200300',
        expect.objectContaining({ content: expect.stringContaining('[Voice message]') }),
      );
    });
    ...
    it('stores audio with placeholder', async () => {
      ...
      const ctx = createMediaCtx({
        extra: { audio: { file_id: 'audio123' } },
      });
      await triggerMediaMessage('message:audio', ctx);

      expect(opts.onMessage).toHaveBeenCalledWith(
        'tg:100200300',
        expect.objectContaining({ content: expect.stringContaining('[Audio]') }),
      );
    });
    ...
    it('stores document with filename', async () => {
      ...
      expect(opts.onMessage).toHaveBeenCalledWith(
        'tg:100200300',
        expect.objectContaining({ content: expect.stringContaining('[Document: report.pdf]') }),
      );
    });
    ...
    it('stores document with fallback name when filename missing', async () => {
      ...
      expect(opts.onMessage).toHaveBeenCalledWith(
        'tg:100200300',
        expect.objectContaining({ content: expect.stringContaining('[Document:') }),
      );
    });
    ...
    it('ignores non-text messages from unregistered chats', async () => {
      ...
      const ctx = createMediaCtx({
        chatId: 999999,
        extra: { photo: [{ file_id: 'photo999', width: 100, height: 100 }] },
      });
      await triggerMediaMessage('message:photo', ctx);
  ...
  describe('bot commands', () => {
    ...
    it('/ping replies with bot status', async () => {
      ...
      expect(ctx.reply).toHaveBeenCalledWith(expect.stringContaining('Online. Uptime:'));
    });
  });
